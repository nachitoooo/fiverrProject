...
def mostrar_telemetria_tiempo_real():
    ...
    for idx, (label_text, data_key) in enumerate([
        ...
        ("Uso de RAM:", "uso_ram"),
        ("Temperatura CPU:", "temperaturas_cpu"),  # Agregado: Temperatura CPU
        ...
        frame = tk.Frame(ventana_telemetria, bg='#0D2E3B', highlightbackground="#3b3c3e", highlightthickness=2, relief="solid", width=200, height=200)  # Fondo azul oscuro, borde redondeado
        frame.grid(row=idx // 2, column=idx % 2, padx=10, pady=10, sticky='nsew')

        title_label = tk.Label(frame, text=label_text, font=('Arial', 18), fg='white', bg='#0D2E3B')  # Texto blanco, fondo azul oscuro, letra bonita
        title_label.pack()

        if data_key == "temperaturas_cpu":
            value_label = tk.Label(frame, textvariable=current_cpu_temperature, font=('Arial', 18), fg='white', bg='#0D2E3B')  # Texto blanco, fondo azul oscuro, letra bonita
        else:
            value_label = tk.Label(frame, text="0%", font=('Arial', 18), fg='white', bg='#0D2E3B')  # Texto blanco, fondo azul oscuro, letra bonita
